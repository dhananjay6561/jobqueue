...
git clone <repo> jobqueue && cd jobqueue
...
3. **Priority** — The sorted-set score encodes both priority and scheduled_at:
   `score = -(priority × 1e12) + scheduledAt.UnixNano`. Higher priority jobs
   always have a smaller (more negative) score and are popped first.

4. **Processing** — After dequeuing a job ID, the worker hydrates the full job
   from PostgreSQL, marks it `running`, and dispatches to the registered handler
   function for that job type.

5. **Heartbeat** — A sub-goroutine per worker updates `last_seen` in PostgreSQL
...
6. **Graceful shutdown** — On SIGTERM, the pool's context is cancelled. Each
   worker's select loop detects cancellation after finishing the current job
   (never mid-job). `Pool.Shutdown()` blocks until all workers return or
   `WORKER_SHUTDOWN_TIMEOUT` elapses.
...
attempts < max_attempts?
    ...
    │               e.g. attempt 1: 5s, 2: 10s, 3: 20s, 4: 40s
    │
...
DLQ entries are not deleted. They can be re-enqueued via
...
- **Job priorities per queue** — currently priority is global; per-queue priority
  bands could improve isolation.
...
- **Multi-tenant queues** — namespace queues by tenant ID with per-tenant rate
  limits.
- **gRPC streaming** — replace the WebSocket events with a gRPC server-stream
  endpoint for internal service consumers.
- **Dead-letter expiry** — automatically purge DLQ entries older than a
  configurable retention period.
- **Job result storage** — store handler return values (not just errors) in the
  payload column for caller retrieval.
